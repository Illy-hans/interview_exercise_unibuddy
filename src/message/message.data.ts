...
export class MessageData {
  ...
  async delete(messageId: ObjectID): Promise<ChatMessage> {
    // The 'deleted' flag needs to be updated to 'true'
    // the chatMessageToObject function updates the text
    // with the default value when the return call is made. 
    const filterBy = {_id: messageId }
    const updateProperty = { deleted: true }
    const deleteMessage = await this.chatMessageModel.findOneAndUpdate(
      filterBy,
      updateProperty,
      {
        new: true,
    ...
    if (!deleteMessage) throw new Error('The message to delete does not exist')
    return chatMessageToObject(deleteMessage); 
  };
